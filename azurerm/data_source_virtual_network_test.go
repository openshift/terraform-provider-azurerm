...
func TestAccDataSourceArmVirtualNetwork_basic_addressPrefixes(t *testing.T) {
	...
	config := testAccDataSourceArmVirtualNetwork_basic_addressPrefixes(ri, testLocation())
...
func testAccDataSourceArmVirtualNetwork_basic_addressPrefixes(rInt int, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
  name     = "acctest%d-rg"
  location = "%s"
